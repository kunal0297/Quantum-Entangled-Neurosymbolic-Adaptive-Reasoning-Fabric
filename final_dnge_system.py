...
import networkx as nx # <-- FIXED: Critical import for graph operations
import time

# Add current directory to path
sys.path.append(os.getcwd())

# --- Conditional Imports (Ensures clean execution flow) ---
try:
    import torch
    ...
    # Assuming normalize_answer is a separate utility
    def normalize_answer(result: Any) -> Any:
        try:
    ...
    print(f"⚠️ CRITICAL IMPORT FAILURE: {e}. Running in Fallback Mode.")
    Decomposer = type('DummyDecomposer', (object,), {'__init__': lambda self: None, 'build_initial_graph': lambda self, *args: None, 'mlp': None, '_embed': lambda self, *args: None})
    execute_graph_topologically = lambda *args: {}
    verification_score = lambda *args: (0.0, {})
    normalize_answer = lambda x: str(x)
    torch = None
    EvolutionConfig = type('DummyConfig', (object,), {'__init__': lambda self: None, 'mutation_rate': 0.3})
# -----------------------------
...
class FinalDNGESystem:
    ...
    def train_system(self, df_train: pd.DataFrame, verbose: bool = False):
        ...
        print("Starting training for Decomposer MLP (5 epochs)...")
        
        learning_rate = 0.001
        epochs = 5
        
        optimizer = torch.optim.Adam(self.decomposer.mlp.parameters(), lr=learning_rate)
        loss_fn = torch.nn.BCELoss() # Binary Cross-Entropy Loss (suitable for multi-label classification)
        ...
        for epoch in range(epochs):
            ...
            for i, row in df_train.iterrows():
                ...
                # Simplified Target Generation (Requires actual labels for optimal results)
                # Target: [math, logic, compare, arith, sequence, confidence] (6 dimensions)
                target_vector = [
                    1.0 if any(k in question.lower() for k in ['sum', 'product']) else 0.0,
                    1.0 if 'if' in question.lower() else 0.0,
                    1.0 if 'greater' in question.lower() else 0.0,
                    1.0 if 'plus' in question.lower() else 0.0,
                    1.0 if 'sequence' in question.lower() else 0.0,
                    1.0 # High confidence target for training stability
                ]
        ...
        self.evolver_config.mutation_rate = 0.40 # Ensure high exploration after training
    ...
    def load_model(self, path: str):
        ...
        try:
            ...
            print(f"Warning: Failed to load model state from {path}. Using default config. Error: {e}")
    ...
    # --- REMAINDER OF FINALDNGE SYSTEM METHODS (As provided previously) ---
